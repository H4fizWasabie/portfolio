...
export default function HeroField() {
  const [enabled, setEnabled] = useState(false); const poster = useRef<HTMLCanvasElement>(null);
  ...
  return enabled ? <Canvas className="hero-poster" frameloop="demand" dpr={[1, 1.5]} camera={{ position: [0, 0, 12] }}><Field /></Canvas> : <canvas className="hero-poster" ref={poster} aria-hidden="true" />;
}
