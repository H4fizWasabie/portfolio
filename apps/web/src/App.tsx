...
const RESUME_PDF = '/resume/Resume-Hafiz-Jamali.pdf';
// Chrome for Android (and most mobile webviews) has no built-in PDF viewer, so
// a plain link to the PDF just downloads (or does nothing). Route visitors to
// /resume, which embeds the PDF through Google Docs Viewer and still offers a
// direct download for desktop viewers.
...
function Shell({ section, children }: { section: string; children: React.ReactNode }) {
  return <div className="mc">
    ...
    <footer className="mc-footer"><div className="wrap">
      ...
      <span><a href="https://github.com/H4fizWasabie">github</a> · <a href="/resume/Resume-Hafiz-Jamali.pdf">résumé</a><a href="https://madewithjev.com/free-tools/ai-slop-detector/report/portfolio.wasabietech.com" style={{display:'inline-block',marginLeft:8,verticalAlign:'middle'}}><img src="https://madewithjev.com/free-tools/ai-slop-detector/badge/portfolio.wasabietech.com" alt="22% AI slop, checked by Jev" height={20} loading="lazy"/></a></span>
    </div></footer>
  </div>;
}
...
function Resume() { return <Shell section="RESUME">
  ...
  <p className="mc-lede">A practical resume for marketing systems, content operations, and AI-assisted workflows.</p>
    <div className="mc-pdf-frame">
    <iframe src={`https://docs.google.com/viewer?url=${encodeURIComponent(location.origin + RESUME_PDF)}&embedded=true`} title="Resume — Hafiz Jamali (PDF)" loading="lazy" />
  </div>
  <div className="mc-pdf-actions">
    <a className="mc-cta" href={RESUME_PDF}>download pdf ↗</a>
    <a className="mc-cta ghost" href={`https://docs.google.com/viewer?url=${encodeURIComponent(location.origin + RESUME_PDF)}&embedded=true`} target="_blank" rel="noreferrer">open in google viewer ↗</a>
  </div>
</Shell>; }
